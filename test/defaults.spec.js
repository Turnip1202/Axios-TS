...
describe('defaults', () => {
    ...
    test('should transform request json', () => {
        expect(axios.defaults.transformRequest[0]({ foo: 'bar' })).toBe('{"foo":"bar"}');
    });
    test('should do nothing to request string', () => {
        expect(axios.defaults.transformRequest[0]('foo=bar')).toBe('foo=bar');
    });
    test('should transform response json', () => {
        const data = axios.defaults.transformResponse[0]('{"foo":"bar"}');
        expect(typeof data).toBe('object');
        expect(data.foo).toBe('bar');
    });
    test('should do nothing to response string', () => {
        expect(axios.defaults.transformResponse[0]('foo=bar')).toBe('foo=bar');
    });
